@props(['settings', 'navBrands', 'bacGroupedProducts'])
      ...
      <div class="group relative">
        ...
        <div id="productsDropdown" class="absolute left-0 top-full z-50 hidden pt-2 group-hover:block"
          ...
          <div class="shadow-soft flex overflow-hidden rounded-xl border border-slate-200 bg-white"
            style="width: 760px; min-height: 240px;">
            ...
            <div id="productsPanel" class="max-h-[360px] flex-1 overflow-y-auto px-5 py-4">
          ...
          @foreach ($navBrands as $brand)
            <div id="brand-products-{{ $brand->id }}" class="hidden">
              ...
              @if ($brand->id == 2 && $bacGroupedProducts)
                {{-- <p class="text-brand mb-4 text-xs font-semibold uppercase tracking-wide">
                  BAC Products
                </p> --}}

                @foreach ($bacGroupedProducts as $group)
                  <div class="mb-5 last:mb-0">

                    <p class="mb-2 text-xs font-semibold uppercase tracking-wide text-red-800">
                      {{ $group['heading'] }}
                    </p>
                    ...
                    <div class="grid grid-cols-2 gap-x-6 gap-y-2">
                      ...
                      @foreach ($group['products'] as $product)
                        <a href="{{ route('products.show', $product->slug) }}"
                          class="hover:text-brand block truncate py-0.5 text-sm text-slate-600 hover:underline">
                          {{ $product->getShortLabel() }}
                        </a>
                      @endforeach
                ...
                <a href="{{ route('products.index') }}?brand=2"
                  class="text-brand hover:text-brand-hover mt-4 block text-xs font-semibold">
                  View all BAC products →
                </a>
              @else
