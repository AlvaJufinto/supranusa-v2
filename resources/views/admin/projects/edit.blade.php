...
@section('content')
  <div class="mb-6">
    ...
    <p class="mt-1 text-sm text-slate-500">
      Update project information, thumbnail, tags, and publication status.
    </p>
  </div>
          ...
          <div class="space-y-5">
            ...
            <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
              ...
              <div>
                ...
                <select id="brand_id" name="brand_id"
                  ...
                  @foreach ($brands as $b)
                    <option value="{{ $b->id }}"
                      {{ old('brand_id', $project->brand_id) == $b->id ? 'selected' : '' }}>
                      {{ $b->name }}
                    </option>
                  @endforeach
                </select>
            ...
            <div>
              ...
              <textarea id="description" name="description" rows="6" placeholder="Describe the project..."
                class="focus:border-brand focus:ring-brand w-full rounded-lg border border-slate-300 px-4 py-2.5 focus:ring-2">{{ old('description', $project->description) }}</textarea>
            </div>
            ...
            @foreach ($tags as $tag)
              <label
                ...
                <input type="checkbox" name="tags[]" value="{{ $tag }}"
                  ...
                  {{ in_array($tag, old('tags', $projectTags)) ? 'checked' : '' }}>
        ...
        <div class="rounded-xl border border-slate-200 bg-white p-6">
          ...
          <div class="mb-4 overflow-hidden rounded-xl border border-slate-200 bg-slate-100">
            @if ($project->thumbnail)
              <img src="{{ asset_url($project->thumbnail) }}" alt="{{ $project->title }}"
          ...
          <label for="thumbnail_file" class="mb-1 block text-sm font-medium text-slate-700">
            {{ $project->thumbnail ? 'Replace Thumbnail' : 'Upload Thumbnail' }}
          </label>
